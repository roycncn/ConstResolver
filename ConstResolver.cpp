...
struct ConstResolver : public ModulePass {
	...
	std::shared_ptr<BitVector> getBitVector(Constant* Const) {
		...
		if (isa<Function>(Const)) {
			errs() << "Skip Function" << "\n";
		//===----------------------------------------------------------------------===//
		// 			This block will handle simple constants type
		...
		//===----------------------------------------------------------------------===//
		// 			This block will handle complex constants type
		// 			(Vector,arrays and structures)
		//===----------------------------------------------------------------------===//
		...
		} else if (ConstantVector* CV = dyn_cast<ConstantVector>(Const)) {
			for (int j = 0; j < CV->getType()->getNumElements(); j++) {
					getBitVector(CV->getOperand(j));
					
						}
		//===----------------------------------------------------------------------===//
		// 			This block will handle GlobalValue
